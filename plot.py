...
model_names = {
    "lfm2.5-instruct:1.2b": "LFM2.5:1.2B-instruct",
...
def plot_by_models(path, col_titles, xlabels=None, selected_models=None, name="models"):
    ...
    xlabels = [model_names[m.replace("-q8_0", "")].replace(":", "\n") for m in models] if not xlabels else xlabels
    ...
    fig, axes = plt.subplots(len(rows_data), n_cols, figsize=(18, 4 * len(rows_data)), squeeze=False, sharey="row", sharex="col")
...
def plot_by_heterogeneity(directory_path, selected_model, devices=None, mutations=None):
    ...
    if isinstance(devices, list) and isinstance(mutations, int):
        ...
        config_labels = [f"{m}% informal" for m in mutations]
        cmap = plt.cm.YlOrBr
    else:
        return
...
def plot_by_hardwares(result_path, selected_models):
    ...
    fig, axes = plt.subplots(1, len(hardwares), figsize=(14, 4), squeeze=False, sharey=None)
    model_labels = [model_names[m.replace("-q4_K_M", "")].replace(":", "\n") + "\nq4_K_M" for m in selected_models]
...
def plot_failure(result_path, selected_models=None):
    ...
    total = sum([count for _, count in display_messages])
...
if __name__ == "__main__":
    ...
    plot_by_models(f"{RESULT_DIR}/{code}/result_D5_M0.csv", col_titles=["Small (<=8B)", "Tiny (<=4B)", "Micro (<=2B)"], selected_models=main_models, name="models")
    plot_by_models(f"{RESULT_DIR}/{code}/result_D5_M0.csv", col_titles=["Qwen3:8B", "Qwen3:4B-instruct", "Qwen3:1.7B", "Qwen3:0.6B"], selected_models=[
        ...
        "qwen3:0.6b-q8_0",
        "qwen3:0.6b-q4_K_M",
    ], name="settings", xlabels=["q8_0\n(reasoning)", "q8_0", "q4_K_M"] * 4)
    
    plot_by_heterogeneity(f"{RESULT_DIR}/{code}", "qwen3:4b-instruct-2507-q8_0", devices=5, mutations=[0, 20, 40, 60, 80, 100])
    plot_by_heterogeneity(f"{RESULT_DIR}/{code}", "qwen3:4b-instruct-2507-q8_0", devices=[5, 10, 15, 20], mutations=0)
