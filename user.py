...
class UserAgent:
    ...
    async def ask(self, user_task):
        coordinator_result = await self.organizer.ainvoke({"user_task": user_task})
    ...
    async def discovery(self, time_to_wait, message):
        ...
        self.client.subscribe(MQTT_TOPIC_TEAM.format(team_id=self.current_team_id))
        self.client.publish(MQTT_TOPIC_DISCOVERY.format(team_id=self.current_team_id), message)
    ...
    def call_agent(self, agent_id, message):
        self.client.publish(MQTT_TOPIC_AGENT.format(agent_id=agent_id), message)

    def on_message(self, client, userdata, message):
        topic, id = message.topic.split("/")

        if check_topic(topic, MQTT_TOPIC_TEAM) and id == self.current_team_id:
